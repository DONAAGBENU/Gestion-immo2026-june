...
const INIT_CONTRACTS: Contract[] = [
  { id:'c1', tenantId:'1', propertyId:'1', type:'bail',        name:'Contrat de bail - Villa Lumière.pdf',     date:'2024-01-15', status:'active',  size:'245 Ko' },
  { id:'c2', tenantId:'2', propertyId:'2', type:'etat_lieux',  name:'État des lieux entrée - Appart Paris.pdf',date:'2024-02-01', status:'active',  size:'1.2 Mo' },
  { id:'c3', tenantId:'3', propertyId:'3', type:'avenant',     name:'Avenant loyer - Studio Lyon.pdf',         date:'2023-11-10', status:'expired', size:'120 Ko' },
  { id:'c4', tenantId:'1', propertyId:'1', type:'autre',       name:'Règlement intérieur.pdf',                 date:'2024-01-15', status:'active',  size:'89 Ko'  },
];
...
export default function Contracts() {
  ...
  const inpS: React.CSSProperties = { background:s.inp, border:`1px solid ${s.inBdr}`, borderRadius:'10px', padding:'9px 12px', color:s.txt, fontSize:'13px', outline:'none', width:'100%' };
  ...
  const filtered = contracts.filter(c => {
    const t = mockTenants.find(t=>t.id===c.tenantId);
    const p = mockProperties.find(p=>p.id===c.propertyId);
  ...
  const generate = () => {
    if (!form.tenantId || !form.propertyId || !form.name.trim()) return;
    setUploading(true);
    setTimeout(() => {
      const newC: Contract = {
  ...
  return (
    <div style={{ minHeight:'100%', background:s.bg, padding:'24px' }}>
      <div style={{ display:'flex', alignItems:'flex-start', justifyContent:'space-between', marginBottom:'24px', flexWrap:'wrap', gap:'12px' }}>
        ...
        <button onClick={()=>setShowModal(true)} style={{ display:'flex', alignItems:'center', gap:'8px', padding:'10px 18px', borderRadius:'12px', background:'linear-gradient(135deg,#3b82f6,#6366f1)', color:'#fff', border:'none', fontWeight:700, fontSize:'13px', cursor:'pointer' }}>
      ...
      <div style={{ position:'relative', marginBottom:'16px', maxWidth:'360px' }}>
        <Search style={{ position:'absolute', left:'12px', top:'50%', transform:'translateY(-50%)', width:'15px', height:'15px', color:s.faint }} />
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder={lang==='en'?'Search documents...':'Rechercher...'} style={{ ...inpS, paddingLeft:'36px' }} />
      </div>
      ...
      <div style={{ display:'flex', flexDirection:'column', gap:'8px' }}>
        {filtered.map(c => {
          ...
          return (
            <div key={c.id} style={{ background:s.card, border:`1px solid ${s.bdr}`, borderRadius:'14px', padding:'14px 16px', display:'flex', alignItems:'center', gap:'12px', flexWrap:'wrap' }}>
              ...
              <div style={{ flex:1, minWidth:0 }}>
                <p style={{ fontSize:'14px', fontWeight:600, color:s.txt, marginBottom:'4px', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{c.name}</p>
                <div style={{ display:'flex', alignItems:'center', gap:'8px', flexWrap:'wrap' }}>
                  ...
                  {tenant && <span style={{ fontSize:'11px', color:s.muted, display:'flex', alignItems:'center', gap:'3px' }}><User style={{ width:'11px', height:'11px' }} />{tenant.firstName} {tenant.lastName}</span>}
                  {property && <span style={{ fontSize:'11px', color:s.muted }}>{property.name}</span>}
                  <span style={{ fontSize:'11px', color:s.faint }}>{new Date(c.date).toLocaleDateString(lang==='en'?'en-GB':'fr-FR')} · {c.size}</span>
                </div>
              </div>
              <div style={{ display:'flex', gap:'6px', flexShrink:0 }}>
                <button title="Télécharger" style={{ padding:'8px', borderRadius:'8px', background:'rgba(59,130,246,0.12)', border:'none', cursor:'pointer', display:'flex', color:'#60a5fa' }}>
                  <Download style={{ width:'15px', height:'15px' }} />
                </button>
                <button onClick={()=>setContracts(prev=>prev.filter(x=>x.id!==c.id))} title="Supprimer" style={{ padding:'8px', borderRadius:'8px', background:'rgba(239,68,68,0.1)', border:'none', cursor:'pointer', display:'flex', color:'#f87171' }}>
        ...
        {filtered.length===0 && (
          <div style={{ textAlign:'center', padding:'60px 0' }}>
            <File style={{ width:'40px', height:'40px', margin:'0 auto 12px', color:s.muted, opacity:.4 }} />
        ...
        <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.75)', backdropFilter:'blur(8px)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:50, padding:'16px' }}>
          <div style={{ background: s.dk ? '#13161f':'#fff', border:`1px solid ${s.bdr}`, borderRadius:'20px', padding:'24px', width:'100%', maxWidth:'460px' }}>
            ...
            <div style={{ display:'flex', flexDirection:'column', gap:'14px' }}>
              <div><label style={lbl}>Nom du document *</label><input value={form.name} onChange={e=>setForm(f=>({...f,name:e.target.value}))} placeholder="Ex: Contrat de bail 2024" style={inpS} /></div>
              ...
              <div><label style={lbl}>Locataire</label>
                <select value={form.tenantId} onChange={e=>setForm(f=>({...f,tenantId:e.target.value}))} style={{ ...inpS, background:s.dk?'#1a1d27':'#f8fafc' }}>
              ...
              <label style={{ display:'flex', flexDirection:'column', alignItems:'center', padding:'24px', borderRadius:'12px', border:`2px dashed ${s.bdr}`, cursor:'pointer', color:s.muted, fontSize:'13px', gap:'6px' }}>
                <Upload style={{ width:'24px', height:'24px' }} />
                <span>Glisser un fichier ou <span style={{ color:'#60a5fa' }}>parcourir</span></span>
                <input type="file" accept=".pdf,.doc,.docx" className="hidden" style={{ display:'none' }} />
              </label>
            </div>
